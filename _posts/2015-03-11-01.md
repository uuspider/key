...
当\\(\frac{\pi d}{\lambda }(\sin \theta - \sin \theta _1 ) = n \pi \\)(\\(n\\)为整数)时，分子、分母同时为零，根据洛必达法则，\\(F(\theta ) = 1 \\)，为最大值。由于\\(n\\)的取值不唯一，\\(F(\theta ) \\)的表现为多瓣状，其中，当\\(n = 0\\)，即\\(\theta = \theta _1\\)时称作主瓣，当\\(n\\)为其它整数时称作栅瓣。栅瓣导致了测角的多值性问题，为了避免栅瓣的出现，需要
...
若\\(\lambda \\)确定，只要调整阵元间距\\(d\\)满足上式，就可以避免栅瓣的出现，通常取\\(\frac{d}{\lambda } \leq \frac{1}{2} \\)。
